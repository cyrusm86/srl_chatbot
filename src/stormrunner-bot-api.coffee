 ...
 #   res.header("Access-Control-Allow-Origin", "*");
  #  res.header("Access-Control-Allow-Headers", "X-Requested-With")
   # res.header("Access-Control-Allow-Methods", "GET,PUT,POST,DELETE")
    ...
    #next()
######################################################################################
  robot.router.post '/hubot/stormrunner/:room', (req, res) ->
    ...
    robot.logger.debug "Recieved body from stormrunner : \n#{JSON.stringify(data)}"

    stormrunnerShareObj = data

    attachments = []
    if stormrunnerShareObj.errorsNetwork!=undefined
      for netErr in stormrunnerShareObj.errorsNetwork
    ...
    errorMessage=""
    if(stormrunnerShareObj.errorMessages != undefined )
      if stormrunnerShareObj.errorsMessages.length>0
        errorMessage=stormrunnerShareObj.errorsMessages[0].message
    msgData = {
      channel: room
      text:"*Failure time*: #{stormrunnerShareObj.crashTime}\n
            *Action*: #{stormrunnerShareObj.action} on #{stormrunnerShareObj.screen}\n
